...
class PolarPlugin : FlutterPlugin, MethodCallHandler, PolarBleApiCallbackProvider, ActivityAware {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            "shutdown" -> shutdownSdk(call, result)
    ...
    private fun createStreamingChannel(call: MethodCall, result: Result) {
        if(api == null) return result.error("SDK not initialized", "Must call init before using the SDK", null)
    ...
    private fun initSdk(call: MethodCall, result: Result) {
        try {
            api = PolarBleApiDefaultImpl.defaultImplementation(
                context, PolarBleSdkFeature.entries.toSet()
            )
    ...
    private fun shutdownSdk(call: MethodCall, result: Result) {
        try {
            api?.shutDown()
            result.success(null)
        } catch (e: Exception) {
            result.error("Could not shutdown Polar SDK", e.message, e.stackTraceToString())
        }
    }
